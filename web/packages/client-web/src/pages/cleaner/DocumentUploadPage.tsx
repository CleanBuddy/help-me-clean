...
function ProfileImageUpload({ avatarUrl, cleanerId, onUploadComplete }: ProfileImageUploadProps) {
  const inputRef = useRef<HTMLInputElement>(null);
  const [uploadAvatar, { loading: uploading }] = useMutation(UPLOAD_CLEANER_AVATAR);
...
interface DocumentCardProps {
  index: number;
  doc: RequiredDoc;
  uploaded?: CleanerDocument;
  cleanerId: string;
  onUploadComplete: () => void;
}

function DocumentCard({ index, doc, uploaded, cleanerId, onUploadComplete }: DocumentCardProps) {
  const inputRef = useRef<HTMLInputElement>(null);
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      await uploadDocument({
        variables: { cleanerId, documentType: doc.type, file },
        refetchQueries: [{ query: MY_CLEANER_PROFILE }],
      });
      onUploadComplete();
      if (inputRef.current) inputRef.current.value = '';
...
export default function DocumentUploadPage() {
  ...
  const { data, refetch } = useQuery(MY_CLEANER_PROFILE);
  ...
  const uploadedDocTypes = documents.map((d: CleanerDocument) => d.documentType);
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="space-y-4 mb-8">
        {REQUIRED_DOCS.map((doc, i) => {
          const uploaded = documents.find((d: CleanerDocument) => d.documentType === doc.type);
          return (
            <DocumentCard
              key={doc.type}
              index={i}
